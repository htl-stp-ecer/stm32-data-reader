...
int32_t rawPress, rawTemp, compHumidity, compTemp, compPress;
// pressure, humidity, and temperature raw count output for BME280
...
float magBias[3] = {-23.15, 359.62, -463.05}, magScale[3] = {1.00, 1.02, 0.98};
...
uint8_t yawBytes[2], pitchBytes[2], rollBytes[2], tempBytes[4], pressBytes[4], humidBytes[4];
// for writing to SPI flash
...
void setup()
{
    ...
    if (d == 0x60)
    {
        // WHO_AM_I register should always return x60 for the BME280
...
void loop()
{
    ...
    if (intFlag == true)
    {
        ...
        if (MPU9250.checkNewAccelGyroData() == true) // data ready interrupt is detected
        {
            ...
            if (MPU9250.checkNewMagData() == true)
            {
    ...
    if (alarmFlag && wakeup)
    {
        // update RTC output (serial display) whenever the RTC alarm condition is achieved and the MPU9250 is awake
        alarmFlag = false;
 ...
 */
        ...
        if (sector_number < 8 && page_number < 0xFFFF)
        {
            // 65,536 256-byte pages in a 16 MByte flash
