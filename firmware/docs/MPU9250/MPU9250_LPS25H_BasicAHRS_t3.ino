/* MPU9250_LPS25H_t3 Basic Example Code
 by: Kris Winer
 date: August 9, 2014
 ...
 This sketch is intended specifically for the MPU9250+LPS25H Add-on shield for the Teensy 3.1.
 It uses SDA/SCL on pins 17/16, respectively, and it uses the Teensy 3.1-specific Wire library i2c_t3.h.
 The LPS25H is a small (2.5 mm x 2.5  mm), low-power, high-resolution (24-bit) pressure sensor, which can be used in its high resolution
 mode but with power consumption of 25 microAmp, or in a lower resolution mode with power consumption of
 only 4 microAmp. The choice will depend on the application.
 
 SDA and SCL should have external pull-up resistors (to 3.3V).
 4K7 resistors are on the MPU9250+LPS25H Mini Add-On board.
 
 Hardware setup:
 MPU9250 Mini Add-On ----- Teensy 3.1
 VDD ---------------------- 3.3V
 SDA ----------------------- A4
 SCL ----------------------- A5
 GND ---------------------- GND
...
// Using the MPU9250+LPS25HMiniTeensy 3.1 Add-On shield, ADO is set to 0 
#define ADO 0
#if ADO
#define MPU9250_ADDRESS 0x69  // Device address when ADO = 1
#define AK8963_ADDRESS 0x0C   // Address of magnetometer
#define LPS25H_ADDRESS 0x5D   // Address of altimeter
#else
...
enum Pavg
{
    // Altimeter pressure internal data averaging
...
enum Tavg
{
    // Altimeter temperature internal data averaging
...
// Bias corrections for gyro, accelerometer, and mag
...
void setup()
{
    ...
    if (c == 0x71) // WHO_AM_I should always be 0x68
    {
        ...
        Serial.print(e, HEX);
        Serial.print(" I should be ");
        Serial.println(0xBD, HEX);
        display.clearDisplay();
        display.setCursor(20, 0);
        display.print("LPS25H");
        display.setCursor(0, 10);
        display.print("I AM");
        display.setCursor(0, 20);
        display.print(e, HEX);
...
void loop()
{
    ...
    if (delt_t > 500)
    {
        ...
        tempCount = readGyroTempData(); // Read the gyro adc values
        gyrotemperature = ((float)tempCount) / 333.87 + 21.0; // Gyro chip temperature in degrees Centigrade
        // Print temperature in degrees Centigrade      
        Serial.print("Gyro temperature is ");
        Serial.print(gyrotemperature, 1);
        Serial.println(" degrees C"); // Print T values to tenths of s degree C

        // Get altimeter data
        if (readByte(LPS25H_ADDRESS, LPS25H_STATUS_REG) & 0x20)
        {
            // check if new altimeter data ready
            Pressure = (float)readAltimeterPressure() / 4096.0f;
            Temperature = (float)readAltimeterTemperature() / 480.0f + 42.5f;
        }
        /*
    const int station_elevation_m = 1050.0f*0.3048f; // Accurate for the roof on my house; convert from feet to meters
...
*/
        ...
        display.print(9. * Temperature / 5. + 32., 0);
        display.setCursor(42, 40);
        display.print((float)sumCount / (1000. * sum), 2);
        display.print("kHz");
        display.display();
