...
void setup()
{
    ...
    if (c == 0xC4) // WHO_AM_I should always be 0xC4
    {
        ...
        Serial.print("Oversampling Ratio is ");
        Serial.println(1 << SAMPLERATE);
        ...
        display.setCursor(0, 10);
        display.print("OSR ");
        display.print(1 << SAMPLERATE);
        display.setCursor(0, 20);
        display.print("Event flags");
        display.setCursor(0, 30);
...
void loop()
{
    ...
    else
    {
        ...
        if (delt_t > 500)
        {
            ...
            const int station_elevation_m = 1050.0 * 0.3048;
            ...
            Serial.println(" Pa"); // Print pressure in Pa
            Serial.print("altitude is ");
            Serial.print(altitude, 2);
            Serial.print(" m, "); // Print altitude in meters   
            Serial.print(altitude / 0.3048, 2);
            Serial.println(" ft"); // Print altitude in feet 
            Serial.print("temperature is ");
            Serial.print(temperature, 2);
            Serial.print(" C, "); // Print temperature in C
            Serial.print((9. * temperature / 5. + 32.), 1);
            Serial.println(" F"); // Print temperature in C
            ...
            display.print(3.28084 * altitude, 1);
            display.print("   ft");
            display.setCursor(0, 40);
            display.print("T   ");
...
void MPL3115A2readAltitude() // Get altitude in meters and temperature in centigrade
{
    ...
    while ((readByte(MPL3115A2_ADDRESS, MPL3115A2_STATUS) & 0x08) == 0);
    MPL3115A2toggleOneShot();

    readBytes(MPL3115A2_ADDRESS, MPL3115A2_OUT_P_MSB, 5, &rawData[0]);
    // Read the five raw data registers into data array
    ...
    if (msbA > 0x7F)
    {
        ...
        altitude = (float)(foo >> 8) + (float)((lsbA >> 4) / 16.0);
        // Whole number plus fraction altitude in meters for negative altitude
        altitude *= -1.;
    }
    else
    {
        altitude = (float)((msbA << 8) | csbA) + (float)((lsbA >> 4) / 16.0);
        // Whole number plus fraction altitude in meters
    }
...
void MPL3115A2readPressure()
{
    ...
    // Combine whole and fractional parts to get entire pressure in Pascal

    // Calculate temperature, check for negative sign
    long foo = 0;
    if (msbT > 0x7F)
    {
        // Is the most significant bit a 1? Then its a negative number in two's complement form
...
void MPL3115A2TimeStep(uint8_t ST_Value)
{
    ...
    if (ST_Value <= 0xF)
    {
        writeByte(MPL3115A2_ADDRESS, MPL3115A2_CTRL_REG2, (c | ST_Value));
        // Set time step n from 0x0 to 0xF (bits 0 - 3) for time intervals from 1 to 32768 (2^n) seconds
    }
