...
uint8_t BMP280Posr = P_OSR_16, BMP280Tosr = T_OSR_02, BMP280Mode = normal, BMP280IIRFilter = BW0_042ODR, BMP280SBy =
            t_62_5ms; // set pressure amd temperature output data rate
...
void setup()
{
    ...
    if (c == 0x71) // WHO_AM_I should always be 0x68
    {
        ...
        Serial.println(1000. * MPU9250accelBias[0]);
        Serial.println(1000. * MPU9250accelBias[1]);
        Serial.println(1000. * MPU9250accelBias[2]);
        Serial.println("gyro biases (dps)");
        Serial.println(MPU9250gyroBias[0]);
        Serial.println(MPU9250gyroBias[1]);
        Serial.println(MPU9250gyroBias[2]);
        ...
        Serial.println(MPU9250magBias[0]);
        Serial.println(MPU9250magBias[1]);
        Serial.println(MPU9250magBias[2]);
...
void loop()
{
    ...
    if (readByte(MPU9250_ADDRESS, MPU9250_INT_STATUS) & 0x01)
    {
        ...
        ax = (float)accelCount[0] * MPU9250aRes - MPU9250accelBias[0];
        ...
        mx = (float)magCount[0] * MPU9250mRes * magCalibration[0] - MPU9250magBias[0];
    ...
    if (delt_t > 500)
    {
        ...
        if (SerialDebug)
        {
            Serial.print("MPU9250 Gyro temperature is ");
...
void MPU9250readGyroData(int16_t* destination)
{
    ...
    readBytes(MPU9250_ADDRESS, MPU9250_GYRO_XOUT_H, 6, &rawData[0]);
...
int16_t MPU9250readTempData()
{
    ...
    readBytes(MPU9250_ADDRESS,MPU9250_TEMP_OUT_H, 2, &rawData[0]);
...
void initAK8963(float* destination)
{
    ...
    writeByte(AK8963_ADDRESS, AK8963_CNTL, MPU9250Mscale << 4 | MPU9250Mmode);
...
void initMPU9250()
{
    ...
    writeByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG, c | MPU9250Ascale << 3);
    ...
    writeByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG2, c & ~0x0F);
    // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])  
    writeByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG2, c | 0x03);
...
void accelgyrocalMPU9250(float* dest1, float* dest2)
{
    ...
    writeByte(MPU9250_ADDRESS, MPU9250_FIFO_EN, 0x78);
...
// Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
    ...
    for (int ii = 0; ii < 200; ii++)
    {
        ...
        readBytes(MPU9250_ADDRESS, MPU9250_ACCEL_XOUT_H, 6, &rawData[0]);
    ...
    writeByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG, 0xE0);
