...
inv_error_t MPU9250_dmpBegin(unsigned short features, unsigned short fifoRate)
{
    ...
    rate = MPU9250_constrain(rate, 1, 200);
    ...
    unsigned char taps, unsigned short tapTime, unsigned short tapMulti)
{
    ...
    {
        axes |= TAP_X;
        xThresh = MPU9250_constrainU16(xThresh, 1, 1600);
    ...
    {
        axes |= TAP_Y;
        yThresh = MPU9250_constrainU16(yThresh, 1, 1600);
    ...
    {
        axes |= TAP_Z;
        zThresh = MPU9250_constrainU16(zThresh, 1, 1600);
...
inv_error_t MPU9250_dmpEnable3Quat(void)
{
    ...
    dmpFeatures = MPU9250_dmpGetEnabledFeatures();
    dmpFeatures &= ~(DMP_FEATURE_6X_LP_QUAT);
    dmpFeatures |= DMP_FEATURE_LP_QUAT;

    if (MPU9250_dmpEnableFeatures(dmpFeatures) != INV_SUCCESS)
        return INV_ERROR;
...
float MPU9250_getPitch(void)
{
    return pitch_inside;
}

float MPU9250_getRoll(void)
{
    return roll_inside;
}

float MPU9250_getYaw(void)
{
    return yaw_inside;
}
