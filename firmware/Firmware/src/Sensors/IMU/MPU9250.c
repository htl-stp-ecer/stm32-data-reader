...
inline GyroRangeInfo getInfoFromRange(const GyroRange range) { return gyroRanges[range]; }
inline AccelRangeInfo getAccelInfoFromRange(const AccelRange range) { return accelRanges[range]; }
inline DLPFBandwidthInfo getDLPFInfoFromRange(const DLPFBandwidth range) { return dlpfBandwidths[range]; }
inline AK8963ModeInfo getAK8963InfoFromMode(const AK8963Mode range) { return ak8963Modes[range]; }
...
void MPU9250_GetData(volatile float* AccData, volatile float* MagData, volatile float* GyroData,
                     volatile float* TemperatureData)
{
    // grab the data from the MPU9250
    readRegisters(ACCEL_OUT, 21, _buffer);

    // combine into 16 bit values
    //convert acclerometer values
    ...
    //convert temperatur values
    rawX = (_buffer[6] << 8 | _buffer[7]);

    *TemperatureData = (float)(rawX - ROOM_TEMP_OFFSET) / TEMPERATUR_SENSOR_SENSITIVITY + 21.0f/*degC*/;
