...
#include "../../../../../Firmware/include/Sensors/IMU/MPU9250.h"
...
#define get_ms      hal_get_tick_count
...
static int setup_compass_spi(void);
...
#endif
#ifdef AK89xx_SECONDARY
static int compass_self_test_spi(void)
{
    unsigned char tmp[6];
    ...
    //power down compass
    writeAK8963Register(AKM_REG_CNTL, AKM_POWER_DOWN);
    //enable self test
    writeAK8963Register(AKM_REG_ASTC, AKM_BIT_SELF_TEST);
    writeAK8963Register(AKM_REG_CNTL, AKM_MODE_SELF_TEST);

    //wait unitl data is ready
    do
    {
        delay_ms(10);
        readAK8963Registers(AKM_REG_ST1, 1, tmp);
    ...
    readAK8963Registers(AKM_REG_HXL, 6, tmp);
...
#endif
AKM_restore :
    writeAK8963Register(AKM_REG_ASTC, 0 | SUPPORTS_AK89xx_HIGH_SENS);
writeAK8963Register(AKM_REG_CNTL, SUPPORTS_AK89xx_HIGH_SENS);
    return result;
}
...
#ifdef AK89xx_SECONDARY
compass_result= compass_self_test_spi();
    if(debug)
...
static int setup_compass_spi(void)
{
    unsigned char data[8], akm_addr = 12;

    mpu_set_bypass(0);

    st.chip_cfg.compass_addr = 12; //default address of Magnetometer

    writeAK8963Register(AKM_REG_CNTL, AKM_POWER_DOWN);
    delay_ms(1);

    writeAK8963Register(AKM_REG_CNTL, AKM_FUSE_ROM_ACCESS);
    delay_ms(1);

    /* Get sensitivity adjustment data from fuse ROM. */
    readAK8963Registers(AKM_REG_ASAX, 3, data);
    st.chip_cfg.mag_sens_adj[0] = (long)data[0] + 128;
    st.chip_cfg.mag_sens_adj[1] = (long)data[1] + 128;
    st.chip_cfg.mag_sens_adj[2] = (long)data[2] + 128;
